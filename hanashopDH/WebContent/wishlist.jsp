...
function goBasket(){
	  ...
	  var form1=document.getElementById("frm1");
		...
		if (flag)
		{
			...
			alert("장바구니에 추가할 목록을 체크해주세요");
		}
		else 
		{
			
			form1.action="wishtobasket.do"
			form1.submit();
		} 
}
...
<%int number123=1; %>
 ...
 <c:forEach var="vo1" items="${requestScope.list}">
 		...
 		<%number123+=1;%>
		<th  style="WIDTH:  160pt; HEIGHT: 40pt">${vo1.w_pname}</th> 
		<th  style="WIDTH:  160pt; HEIGHT: 40pt">${vo1.w_count }</th>
		<th  style="WIDTH:  160pt; HEIGHT: 40pt">${vo1.w_size }</th>
		<th  style="WIDTH:  160pt; HEIGHT: 40pt">${vo1.w_price }</th>							<!-- 아이디,상품명,사이즈,개수,가격,상품번호 -->	
		<th  style="WIDTH:  160pt; HEIGHT: 40pt"><input type="checkbox" name="deleteList" value="${id },${vo1.w_pname},${vo1.w_size },${vo1.w_count },${vo1.w_price },${vo1.w_no }"></th>
 		</tr>
 </c:forEach>
<!--  <tr>
 ...
 <%		 
		 ...
		 if(groupnum>1){//그룹이 1보다 큰 경우에만 이전버튼 출력
			...
			out.print("<a href='wishList.do?start="+nexts2+"&end="+nexte2+"&currentgroup="+nextg2+"'>이전</a>&nbsp&nbsp");
		 }	 
 		 
 			
 			for(int i=groupsize*(groupnum-1);i<(groupsize*(groupnum-1)+groupsize);i++){//현재 그룹에 따른 페이지버튼 숫자를 설정
 	 			out.print("<a href='wishList.do?start="+number+"&end="+number2+"&currentpage="+(i+1)+"&currentgroup="+groupnum+"'>"+"["+(i+1)+"]</a>&nbsp&nbsp");
 		 ...
 		 if(groupnum!=totgroup){//현재 그룹이 총 그룹인경우 다음버튼 출력하지 않음
 			...
 			out.print("<a href='wishList.do?start="+nexts+"&end="+nexte+"&currentgroup="+nextg+"'>다음</a>");
 		 }
 %>
 <br>
 <button type="button" name=option12  id=btn1  onclick="goBasket()" >장바구니에 추가</button>
 <button type="button" name=option12 id=btn1 onclick="delete12()">삭제하기</button><br>

 <input type=hidden name=point111 value="${point }">
</center>
</form>
