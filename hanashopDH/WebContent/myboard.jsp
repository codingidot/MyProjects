...
%>
	<table>
	<tr><td height=100></tr>		
	 <c:if test="${inReview !='no'}">
	</table>
	<table id="table1" border=1>	
	<%int num=0; %>
	<tr> <th id="th1" style="WIDTH:  100pt; HEIGHT: 40pt; background-color:#ffe5dd ">번호</th>
			<th style="WIDTH: 300pt; HEIGHT: 40pt;  background-color:#ffe5dd ">제목</th>
			<th  id="th3" style="WIDTH: 200pt; HEIGHT: 40pt; background-color:#ffe5dd ">작성자</th>
	</tr>	
	...
	<tr><th style="WIDTH:  200pt; HEIGHT: 40pt"><%=num%></th>
 	...
 	<a href="boardinfo.do?no=${vo.r_no}">${vo.r_title }</a></th>
	<th style="WIDTH:  200pt; HEIGHT: 40pt">${vo.r_id }</th>
	</tr>
	</c:forEach>
	...
	<%		 
		 ...
		 if(groupnum>1){//그룹이 1보다 큰 경우에만 이전버튼 출력
			...
			out.print("<a href='myboard.do?start="+nexts2+"&end="+nexte2+"&currentgroup="+nextg2+"'>이전</a>&nbsp&nbsp");
		 }	 
 		 
 			
 			for(int i=groupsize*(groupnum-1);i<(groupsize*(groupnum-1)+groupsize);i++){//현재 그룹에 따른 페이지버튼 숫자를 설정
 	 			
 	 			out.print("<a href='myboard.do?start="+number+"&end="+number2+"&currentpage="+(i+1)+"&currentgroup="+groupnum+"'>"+"["+(i+1)+"]</a>&nbsp&nbsp");
 		 ...
 		 if(groupnum!=totgroup){//현재 그룹이 총 그룹인경우 다음버튼 출력하지 않음
 			...
 			out.print("<a href='myboard.do?start="+nexts+"&end="+nexte+"&currentgroup="+nextg+"'>다음</a>");
 		 }
 %>
	</c:if>
