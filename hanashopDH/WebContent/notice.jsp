...
%>
	...
	<tr><th style="WIDTH:  200pt; HEIGHT: 40pt"><%=num%></th>
 	...
 	<a href="noticeinfo.do?no=${vo.n_no}">${vo.n_title }</a></th>
	<th style="WIDTH:  200pt; HEIGHT: 40pt">관리자</th>
</tr>
	</c:forEach>
	</table><%		 
		 ...
		 if(groupnum>1){//그룹이 1보다 큰 경우에만 이전버튼 출력
			...
			out.print("<a href='notice.do?start="+nexts2+"&end="+nexte2+"&currentgroup="+nextg2+"'>이전</a>&nbsp&nbsp");
		 }	 
 		 
 			
 			for(int i=groupsize*(groupnum-1);i<(groupsize*(groupnum-1)+groupsize);i++){//현재 그룹에 따른 페이지버튼 숫자를 설정
 	 			
 	 			out.print("<a href='notice.do?start="+number+"&end="+number2+"&currentpage="+(i+1)+"&currentgroup="+groupnum+"'>"+"["+(i+1)+"]</a>&nbsp&nbsp");
 		 ...
 		 if(groupnum!=totgroup){//현재 그룹이 총 그룹인경우 다음버튼 출력하지 않음
 			...
 			out.print("<a href='notice.do?start="+nexts+"&end="+nexte+"&currentgroup="+nextg+"'>다음</a>");
 		 }
 %>
	</c:if>
